...
//
// V1u: ordered (o) - no ordering heuristic
//      simple pruning (1)
//
// compile: javac -cp .. NumPartV1u.java
// run:     java -cp .. numpart.NumPartV1u PROBLEM_FILE [OPTIONS]
...
public class NumPartV1u extends OptInstance<NumPartV1u.Node, Long>
{
  ...
  public int prune(Node current, Node incumbent) {
    if (incumbent.part.length == incumbent.np.n())
      if (current.msum >= incumbent.msum)
        return 1;  // prune: current partial solution can't beat incumbent
    return 0;
  }
  ...
  private NumPartV1u(Node root, Generator<Node> gen0) {
    super(root, gen0);
    this.np = root.np;
  }

  // instance factory; returns null if np is null.
  public static NumPartV1u mkNumPart(NumPartInstance np) {
    if (np == null)
      return null;
    ...
    // return new instance
    return new NumPartV1u(root, gen0);
  }
