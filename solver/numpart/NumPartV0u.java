...
//
// V0u: ordered (o) - no ordering heuristic
//      no pruning (0)
//
// compile: javac -cp .. NumPartV0u.java
// run:     java -cp .. numpart.NumPartV0u PROBLEM_FILE [OPTIONS]
...
public class NumPartV0u extends OptInstance<NumPartV0u.Node, Long>
{
  ...
  static class NodeGenerator implements Generator<Node>
  {
    ...
    public Node next() {
      ...
      // construct next node:
      // (1) copy parent partition and add level-th number, x, to k-th part,
      long x = np.s(level);
      int[] part = Arrays.copyOf(parent.part, level + 1);
      part[level] = k;

      // (2) copy parent sums and move level-th number, x, from rsum to k-th sum
  ...
  /////////////////////////////////////////////////////////////////////////////
  // No pruning: Using default prune() method inherited from OptInstance<>
  ...
  private NumPartV0u(Node root, Generator<Node> gen0) {
    super(root, gen0);
    this.np = root.np;
  }

  // instance factory; returns null if np is null.
  public static NumPartV0u mkNumPart(NumPartInstance np) {
    if (np == null)
      return null;
    ...
    // return new instance
    return new NumPartV0u(root, gen0);
  }
